...
?>
            <h1 class="page-header">Dist-Upgrades <small><a href="#"><div onClick="pageLoad('distupgrade');" class="fa fa-refresh rotate"></div></a>  &nbsp;&nbsp;&nbsp;<button onClick="apt_update();" class="btn btn-raised btn-info">Get Updates</button> &nbsp;&nbsp;&nbsp;<button onClick="apt_distupgrade();" class="btn btn-raised btn-<?php if (($updates_count == 0)  && ($installs_count == 0) && ($removes_count == 0)) {
    echo "outline-";
    	...
    	<div class="col-lg-12">
            ...
            echo "<h5><span class=\"label label-danger\" > <span class=\"glyphicon glyphicon-exclamation-sign\" aria-hidden=\"true\"></span> Please review carefully the table below before starting the Dist-Upgrade</span></h5>";
                ...
                foreach ($updates_array as $upd) {
                    ...
                    $u_new = $upd['u_new'];
                    $u_arch = $upd['u_arch'];
                    $p_full = "/var/cache/apt/archives/".$u_name."_".$u_new."_".$u_arch.".deb";
                    echo '<tr><td>'.$u_name.'</td><td>'.$u_installed.'</td><td>'.$u_new.'</td></tr>'."\n";
                    if ( file_exists($p_full) ) {
                        $p_dlcount++;
                        echo '<tr><td colspan="3">'.$p_full.'</td></tr>'."\n";
                        if ( $p_changes = shell_exec("sudo /usr/bin/apt-listchanges -f text ".$p_full) ){
                        echo '<tr><td colspan="3">'.nl2br($p_changes).'</td></tr>'."\n";
                        }
                    }
                ...
                if ( ( $p_updcount > 0 ) && ( $p_dlcount == 0 )) {
                    echo '<button onClick="apt_downloadupdates();" class="btn btn-raised btn-info">Preload updated packages</button>'."\n";
                }
            ?>
    	</div>
    </div>
